...
class RemoteOrbitalPaymentechTest < Test::Unit::TestCase
  ...
  def test_authorize
    if @certifying
      puts "Authorize:"
      with_cert_data(:auth) {|amt, cc, opts| @gateway.authorize(amt, cc, opts)}
    else
  ...
  def test_purchase
    if @certifying
      puts "Purchase:"
      with_cert_data(:auth_capture) {|amt, cc, opts| @gateway.purchase(amt, cc, opts)}
    else
      with_test_data do |amt, cc, opts, tests|
        ...
        AVS_TEST_ZIPCODES.each do |avsdata|
          CVV_TEST_CODES.each do |cvvdata|
            test_cards.each do |cardtype, number|
              ...
              tests = lambda do |response|
                amtdata[:amount] == 100 ? assert_success(response, debug_values) : assert_failure(response, debug_values)
